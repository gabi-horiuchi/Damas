...
class Jogo:
	...
	def desenha(self):
		...
		if self.cedula_selecionada:
			...
			if obrigatorios != {}:
				if (self.cedula_selecionada[0], self.cedula_selecionada[1]) not in obrigatorios:
					...
					pygame.draw.rect(display, VERMELHO, (x_vermelho, y_vermelho, 75, 75))
				else:
					if movs[0] == []:
						x_vermelho = altura / 8 * self.cedula_selecionada[1]
						y_vermelho = altura/ 8 * self.cedula_selecionada[0]
						...
						for i in range(len(movs[0])):
							...
							pygame.draw.rect(display, VERDE, (x_possivel, y_possivel, 75, 75))
			else:
				if self.pulando:
					x_vermelho = altura / 8 * self.cedula_selecionada[1]
					y_vermelho = altura / 8 * self.cedula_selecionada[0]
		...
		for l in range(len(self.matriz_jogadores)):
			for c in range(len(self.matriz_jogadores[l])):
				...
				if elemento != '-':
					...
					if elemento.lower() == 'x':
						pygame.draw.circle(display, ROXO, (x, y), 20, 0)
						if elemento == 'X':
							pygame.draw.circle(display, PRETO, (x, y), 10, 0)
							pygame.draw.circle(display, AZUL, (x, y), 5, 0)
					else:
						pygame.draw.circle(display, LARANJA, (x, y), 20, 0)
						if elemento == 'O':
							pygame.draw.circle(display, PRETO, (x, y), 10, 0)
							pygame.draw.circle(display, AZUL, (x, y), 5, 0)
		...
		if self.status != 'Game Over':
			...
			surface_texto, rect_texto = text_objects("Roxo: " + str(12 - o), fonte, ROXO)
			rect_texto.center = (650, 30)
			display.blit(surface_texto, rect_texto)

			surface_texto, rect_texto = text_objects("Laranja: " + str(12 - x), fonte, LARANJA)
			rect_texto.center = (650, altura - 30)
			display.blit(surface_texto, rect_texto)

			if self.turno % 2 == 1:
				surface_texto, rect_texto = text_objects("Turno do Laranja", fonte, LARANJA)
				rect_texto.center = (700, altura / 2)
				display.blit(surface_texto, rect_texto)
			else:
				surface_texto, rect_texto = text_objects("Turno do Roxo", fonte, ROXO)
				rect_texto.center = (700, altura / 2)
				display.blit(surface_texto, rect_texto)
		else:
...
def cria_botao(msg, sqr, cor1, cor2, cor_texto, acao=None):
	...
	surface_texto, rect_texto = text_objects(msg, fonte, cor_texto)
...
def regras():
	...
	while not sair:
		...
		info5 = fonte.render('A dama eh uma peca de movimentos mais amplos. Ela anda para frente e para tras,', False, (BRANCO))
		...
		info8 = fonte.render('Duas ou mais pecas juntas, na mesma diagonal, nao podem ser capturadas.', False, (BRANCO))
		...
		game1 = fonte.render('Durante o jogo, ao clicar em uma peca, sera exibido em verde os movimentos', False, (VERDE))
		...
		voltar = fonte.render('Pressione ESC para voltar ao menu.', False, AZUL)
...
def tela_vencedor(vencedor):
	...
	while not sair:
		...
		if vencedor == "empate":
			...
			surface_texto, rect_texto = text_objects("VITORIA DO  ROXO", fonte, ROXO)
		elif vencedor == "o":
			surface_texto, rect_texto = text_objects("VITORIA DO VERDE", fonte, VERDE)

		rect_texto.center = ((largura / 2),altura / 3)
		display.blit(surface_texto, rect_texto)

		fonte = pygame.font.Font(None, 30)
		voltar = fonte.render('Pressione ESC para voltar ao menu.', False, VERDE)
...
def menu_jogo():
	while True:
		...
		cria_botao("INICIAR",(largura - 650, altura / 2, 120, 60), BRANCO, AZUL, PRETO, loop_jogo)
		cria_botao("MANUAL",(largura - 450, altura / 2, 120, 60),BRANCO, AZUL, PRETO, regras)
		cria_botao("SAIR",(largura - 250, altura / 2, 120, 60), BRANCO, AZUL, PRETO, sair)
